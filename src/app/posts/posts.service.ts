...
export class PostsService {
    ...
    getPosts(postsPerPage: number, currentPage: number) {
        ...
        .pipe(map((postData) => {
            ...
            this.postsUpdated.next({posts: [...this.posts], postCount: updatedPosts.maxPosts})
        })
    }
    ...
    addPost(title:string, content: string, image: File) {
        ...
        this.http.post<{message: string, post:Post}>("http://localhost:3001/api/posts", postData)
        .subscribe((resData) => {
    ...
    updatePost(id:string, title: string, content: string, image: File | string) {
        ...
        this.http.put("http://localhost:3001/api/posts/" + id, postData)
            .subscribe((res) => {
